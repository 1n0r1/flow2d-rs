        ...
        for x in 0..space_size[0] {
            for y in 0..space_size[1] {
                match self.space_domain.get_cell(x, y).cell_type {
                    CellType::FluidCell => {
                        let right_cell_type: Option<CellType> = (x + 1 < space_size[0]).then(|| self.space_domain.get_cell(x+ 1, y).cell_type);
                        let top_cell_type: Option<CellType> = (y + 1 < space_size[1]).then(|| self.space_domain.get_cell(x, y + 1).cell_type);
        
                        if let Some(right_cell_type) = right_cell_type {
                            if let CellType::BoundaryConditionCell(_) = right_cell_type {
                            
                            } else {
                                self.space_domain.get_cell_mut(x, y).velocity[0] = self.space_domain.get_cell(x, y).f - self.delta_time*(self.space_domain.get_cell(x+ 1, y).pressure - self.space_domain.get_cell(x, y).pressure)/delta_space[0]
                            }
                        }
                        ...
                        if let Some(top_cell_type) = top_cell_type {
                            if let CellType::BoundaryConditionCell(_) = top_cell_type {
                                ...
                                self.space_domain.get_cell_mut(x, y).velocity[1] = self.space_domain.get_cell(x, y).g - self.delta_time*(self.space_domain.get_cell(x, y + 1).pressure - self.space_domain.get_cell(x, y).pressure)/delta_space[1]
                            }
            ...
            for x in 0..space_size[0] {
                for y in 0..space_size[1] {
                    match self.space_domain.get_cell(x, y).cell_type {
                        CellType::FluidCell => {
                            residual_norm += (
                                (self.space_domain.get_cell(x+ 1, y).pressure - 2.0*self.space_domain.get_cell(x, y).pressure + self.space_domain.get_cell(x - 1, y).pressure 
                                )/delta_space[0].powi(2)
                                + (self.space_domain.get_cell(x, y + 1).pressure - 2.0*self.space_domain.get_cell(x, y).pressure + self.space_domain.get_cell(x, y - 1).pressure 
                    ...
                    match self.space_domain.get_cell(x, y).cell_type {
                        CellType::FluidCell => {
                            self.space_domain.get_cell_mut(x, y).pressure = 
                                (1.0 - OMEGA)*self.space_domain.get_cell(x, y).pressure + OMEGA*(
                                    (self.space_domain.get_cell(x+ 1, y).pressure + (self.space_domain.get_cell(x - 1, y).pressure))/delta_space[0].powi(2)
                                    + (self.space_domain.get_cell(x, y + 1).pressure + (self.space_domain.get_cell(x, y - 1).pressure))/delta_space[1].powi(2)
                                    - self.space_domain.get_cell(x, y).rhs
                                )
            ...
            for y in 0..space_size[1] {
                ...
                if let CellType::BoundaryConditionCell(_) = cell_type {
                    ...
                    for (dx, dy) in neighboring_cells.iter() {
                        if let Some(cell) = self.space_domain.get_space().get(*dx).and_then(|row| row.get(*dy)) {
                    ...
                    if neighboring_fluid_count != 0 {
                        self.space_domain.get_cell_mut(x, y).pressure = self.space_domain.get_cell(x, y).pressure/(neighboring_fluid_count as f32);
                    }
                }
            }
        }
                ...
                match self.space_domain.get_cell(x, y).cell_type {
                    CellType::FluidCell => {
                        self.space_domain.get_cell_mut(x, y).rhs = (
                            (self.space_domain.get_cell(x, y).f - self.space_domain.get_cell(x - 1, y).f)/delta_space[0]
                            + (self.space_domain.get_cell(x, y).g - self.space_domain.get_cell(x, y - 1).g)/delta_space[1]
                        )/self.delta_time;
                    },
                    _ => { }
                }
                    ...
                    CellType::FluidCell => {
                        ...
                        if let Some(right_cell_type) = right_cell_type {
                            if let CellType::BoundaryConditionCell(_) = right_cell_type {
                                ...
                                self.space_domain.get_cell_mut(x, y).f = self.space_domain.get_cell(x, y).velocity[0]
                                + self.delta_time*(
                                    (self.space_domain.d2udx2(x, y) + self.space_domain.d2udy2(x, y))/self.reynolds
                                    - self.space_domain.du2dx(x, y) - self.space_domain.duvdy(x, y) + self.acceleration[0]
                                );
                            }
                        }
                        ...
                        if let Some(top_cell_type) = top_cell_type {
                            if let CellType::BoundaryConditionCell(_) = top_cell_type {
                                ...
                                self.space_domain.get_cell_mut(x, y).g= self.space_domain.get_cell(x, y).velocity[1]
                                + self.delta_time*(
                                    (self.space_domain.d2vdx2(x, y) + self.space_domain.d2vdy2(x, y))/self.reynolds
                                    - self.space_domain.duvdx(x, y) - self.space_domain.dv2dy(x, y) + self.acceleration[1]
                                )
                            }
